...
function isAuthValid(): void {
    ...
    if (key) {
        let response = UrlFetchApp.fetch('https://api.similarweb.com/capabilities?api_key=' + key, { muteHttpExceptions: true });
...
function checkForValidKey(key: string): boolean {
    ...
    let data = httpGet(buildUrl('https://api.similarweb.com/capabilities', { 'api_key':  key}));
...
function setCredentials(request) {
    ...
    let isValid = checkForValidKey(key);
    if (!isValid) {
        return {
...
enum EndpointType {
    Web_Desktop_Data = 'web_desktop_data',
    Web_Mobile_Data = 'web_mobile_data',
    App_Data = 'app_data',
    App_Engagment_Data = 'app_engagement_data'
}
...
function gatherUrls(requestedFields:GoogleAppsScript.Data_Studio.Fields, domains: string[], country: string): string[] {
    ...
    const desktopParams = configurator.getDefaultParams(EndpointType.Web_Desktop_Data, country);
    const mobileParams = configurator.getDefaultParams(EndpointType.Web_Mobile_Data, country);
...
function prepareData(responses: UrlDataMap): object {
    ...
    Object.keys(responses).forEach((url: string) => {
...
function buildTabularData(requestedFields: GoogleAppsScript.Data_Studio.Fields, preparedData): any[][] {
    let requestedData = [];
    Object.keys(preparedData).forEach((dom): void => {
        Object.keys(preparedData[dom]).forEach((device) => {
            Object.keys(preparedData[dom][device]).forEach((date) => {
...
function getData(request) {
    ...
    let country = request.configParams.country.trim().toLowerCase() as string;
    let domains = request.configParams.domains.split(',').slice(0, MAX_NB_DOMAINS).map(cleanDomain) as string[];
    ...
    let requestedFields = getConnectorFields().forIds(requestedFieldIDs);
    ...
    console.log('requested fields - ', JSON.stringify(requestedFields.asArray().map(x => x.getId())));
    console.log('tabular data - ', JSON.stringify(tabularData.slice(0, 5)));
